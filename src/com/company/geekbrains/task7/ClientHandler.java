    ...
    public void readMessage() throws IOException {
        while (true) {
            ...
            if (message.equals("/end")) {
                server.setTextToChatArea(String.format("%s покинул чат", record.getName()));
                server.broadcastMessage("/end");
                server.unsubscribe(this);
                return;
            }
            server.setTextToChatArea(String.format("Incoming message from %s: %s", record.getName(), message));
